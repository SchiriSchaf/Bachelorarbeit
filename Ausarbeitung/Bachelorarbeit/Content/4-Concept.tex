...
\newline
Die Beiden Programme werden im folgenden als Helper und Renderer erwähnt. Weiterhin wurde ein Pythonskript \textit{PlotHelper} geschrieben, um LH-Histogramme anzuzeigen.
\newline
\todo{Christians Paper}
\todo{Unity genauer erklären, Darstellung des Daten, Benutzeroberfläche etc.}
...
Anschließend geht es in der \textit{SpatialClustering} Klasse mit der Berechnung der räumlichen Cluster weiter. Diese werden auf den Übergebenen Listen direkt  kalkuliert. Hierbei werden gefundene Cluster direkt aus der Liste entfernt.
\newline
Als Ergebnis wird erneut eine Liste von Listen vom Typ \textit{IVector3} zurückgegeben.
\todo{Räumlich Clustern beschreiben}
